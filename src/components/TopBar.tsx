...
export const TopBar: React.FC<TopBarProps> = ({
  ...
  return (
    <header className="h-14 bg-white border-b border-slate-200 px-6 flex items-center justify-between sticky top-0 z-30">
      ...
      <div className="relative flex-1 max-w-lg" ref={searchContainerRef}>
        <div className="relative w-full">
          ...
          <input
            ...
            className="w-full bg-slate-50 hover:bg-slate-100/80 focus:bg-white border border-slate-200 focus:border-slate-400 rounded-lg pl-9 pr-14 py-1.5 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-900/10 transition-all"
          />
          ...
          <div className="absolute right-2.5 top-1/2 -translate-y-1/2 flex items-center gap-1.5">
            {searchVal ? (
              ...
              <kbd className="hidden sm:inline-flex items-center gap-0.5 text-[10px] text-slate-400 bg-white px-1.5 py-0.5 rounded border border-slate-200 font-mono shadow-2xs">
                <span>⌘</span>K
              </kbd>
            )}
          </div>
        ...
        {isOpen && (
          <div className="absolute left-0 top-full mt-2 w-full sm:w-[540px] max-w-[90vw] bg-white rounded-xl shadow-2xl border border-slate-200 overflow-hidden z-50 animate-in fade-in zoom-in-95 text-xs">
            {/* Category Filter Chips */}
            <div className="flex items-center gap-1.5 px-3 py-2 border-b border-slate-100 bg-slate-50/60 overflow-x-auto">
      ...
      {/* Right: Date Range, Live Status, Reset, Notifications */}
      <div className="flex items-center gap-3">
        {/* Date Range Selector */}
        <div className="hidden sm:flex items-center p-0.5 bg-slate-100 rounded-lg border border-slate-200/80 text-xs">
        ...
        <div className="hidden lg:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-[11px] font-medium text-emerald-800">
              ...
              <div className="divide-y divide-slate-100 mt-2 space-y-1 max-h-72 overflow-y-auto">
                {notifications.map((n) => (
                  <div
                    ...
                    className="p-2 hover:bg-slate-50 rounded-lg cursor-pointer transition-colors text-left"
                  >
        ...
        >
          <RefreshCw className={cn('w-4 h-4', isResetting && 'animate-spin text-slate-600')} />
        </button>
      </div>
    </header>
  );
